...
def show_ai_validation_page():
    ...
    # AI validation checks with their associated prompts
    ...
    if st.button("🧠 Run selected AI validation prompts"):
        with st.spinner("Sending selected prompts to Mistral..."):
            ...
            # Get and safely parse sample data
            raw_samples = loader.get_sample_data(5)
            parsed_samples = []
            for sample in raw_samples:
                try:
                    parsed = json.loads(sample) if isinstance(sample, str) else sample
                except Exception:
                    parsed = {"text": str(sample)}
                parsed_samples.append(parsed)

            # Use the first valid sample for all prompts
            example_text = None
            for sample in parsed_samples:
                example_text = sample.get("text") or sample.get("content") or str(sample)
